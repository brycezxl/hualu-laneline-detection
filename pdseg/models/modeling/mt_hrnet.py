...
def high_resolution_net(input, num_classes):
    ...
    out1 = fluid.layers.conv2d(
        ...
        bias_attr=False)
    out1 = fluid.layers.resize_bilinear(out1, input.shape[2:])
    ...
    out2 = fluid.layers.conv2d(
        ...
        padding=0,
        act=None,
        param_attr=ParamAttr(initializer=MSRA(), name='conv-11_weights'),
        bias_attr=False)
    out2 = fluid.layers.resize_bilinear(out2, input.shape[2:])

    return out1, out2


def mt_hrnet(input, num_classes):
